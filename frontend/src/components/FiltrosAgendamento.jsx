import React from "react";
import { Search, SlidersHorizontal, X } from "lucide-react";

export default function FiltrosAgendamento({ filtros, onMudarFiltros }) {
  return (
    <div className="app-surface p-4">
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input
            type="text"
            placeholder="Buscar por nome ou serviço..."
            value={filtros.busca || ""}
            onChange={(e) => onMudarFiltros({ ...filtros, busca: e.target.value })}
            className="app-input pl-10"
          />
        </div>

        <select
          value={filtros.status || "todos"}
          onChange={(e) => onMudarFiltros({ ...filtros, status: e.target.value })}
          className="app-input w-full md:w-48"
        >
          <option value="todos">Todos os Status</option>
          <option value="pendente">Pendente</option>
          <option value="confirmado">Confirmado</option>
          <option value="concluido">Concluído</option>
          <option value="cancelado">Cancelado</option>
        </select>

        <select
          value={filtros.periodo || "todos"}
          onChange={(e) => onMudarFiltros({ ...filtros, periodo: e.target.value })}
          className="app-input w-full md:w-48"
        >
          <option value="todos">Todos os Períodos</option>
          <option value="hoje">Hoje</option>
          <option value="amanha">Amanhã</option>
          <option value="semana">Esta Semana</option>
          <option value="mes">Este Mês</option>
        </select>
        {(filtros.busca || filtros.status !== "todos" || filtros.periodo !== "todos") && <button type="button" onClick={() => onMudarFiltros({ busca: "", status: "todos", periodo: "todos" })} className="btn-secondary px-4" title="Limpar filtros"><X className="h-4 w-4" />Limpar</button>}
      </div>
    </div>
  );
}
