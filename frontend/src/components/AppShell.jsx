    ...
    <div className="min-h-screen bg-[#f6f7fb] lg:flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-slate-950 text-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${aberto ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-20 items-center justify-between border-b border-white/10 px-6">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-500"><CalendarDays className="h-5 w-5" /></span><div><p className="font-bold tracking-tight">{brand.name}</p><p className="text-xs text-slate-400">Gestão de horários</p></div></div>
      ...
      <div className="min-w-0 flex-1"><header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur lg:hidden"><button onClick={() => setAberto(true)} className="rounded-xl border border-slate-200 p-2.5"><Menu className="h-5 w-5" /></button><span className="ml-3 font-bold">{brand.name}</span></header>{children}</div>
    </div>
  );
}
