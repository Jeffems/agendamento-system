...
import { Briefcase, Building2, Check, Clock, Copy, Edit3, ExternalLink, Link, Plus, Save, Trash2, X } from "lucide-react";
...
export default function Configuracoes() {
  ...
  const [config, setConfig] = useState({ nome_negocio: "", telefone_negocio: "", endereco_negocio: "", logo_url: "", timezone: "America/Cuiaba", horarios_funcionamento: horariosPadrao, lembrete_email_ativo: true, lembrete_whatsapp_ativo: true, slug: "", agenda_publica_ativa: false, intervalo_agendamento_min: 15, antecedencia_min_horas: 2, limite_agendamento_dias: 60 });
  ...
  return <AppShell><main className="mx-auto max-w-[1200px] px-4 py-7 sm:px-6 lg:px-10 lg:py-10">
    ...
    <div className="mb-6 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-1.5">
      {[["negocio", Building2, "Meu negócio"], ["servicos", Briefcase, "Serviços"], ["horarios", Clock, "Horários"], ["agenda", Link, "Agenda pública"]].map(([id, Icon, nome]) => <button key={id} onClick={() => setAba(id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold ${aba === id ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}><Icon className="h-4 w-4"/>{nome}</button>)}
    </div>
    ...
    </section> : <form onSubmit={salvarConfig} className="app-surface p-6 sm:p-8">{aba === "negocio" ? <><div className="mb-6"><h2 className="text-xl font-bold">Identidade do negócio</h2><p className="text-sm text-slate-500">Essas informações identificam sua empresa dentro da plataforma.</p></div><div className="grid gap-5 md:grid-cols-2">{campo("nome_negocio", "Nome do negócio", "Ex.: Studio Mariana")}{campo("telefone_negocio", "Telefone comercial", "(66) 99999-9999")}{campo("endereco_negocio", "Endereço", "Rua, número, bairro e cidade")}{campo("logo_url", "URL do logotipo", "https://...")}</div><div className="mt-7 grid gap-4 md:grid-cols-2"><Toggle label="Lembretes por e-mail" ativo={config.lembrete_email_ativo} onChange={(v) => setConfig({ ...config, lembrete_email_ativo: v })}/><Toggle label="Lembretes por WhatsApp" ativo={config.lembrete_whatsapp_ativo} onChange={(v) => setConfig({ ...config, lembrete_whatsapp_ativo: v })}/></div></> : aba === "agenda" ? <><div className="mb-6"><h2 className="text-xl font-bold">Agenda pública</h2><p className="text-sm text-slate-500">Crie um link para seus clientes escolherem serviços e horários.</p></div><Toggle label="Aceitar agendamentos pelo link público" ativo={config.agenda_publica_ativa} onChange={(v) => setConfig({ ...config, agenda_publica_ativa: v })}/><div className="mt-6 grid gap-5 md:grid-cols-2"><label><span className="mb-2 block text-sm font-semibold">Endereço personalizado</span><div className="flex"><span className="flex items-center rounded-l-xl border border-r-0 border-slate-200 bg-slate-50 px-3 text-sm text-slate-500">/agendar/</span><input className="app-input rounded-l-none" value={config.slug || ""} placeholder="studio-mariana" onChange={(e) => setConfig({ ...config, slug: e.target.value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") })}/></div></label>{campo("intervalo_agendamento_min", "Intervalo entre atendimentos (min)", "15", "number")}{campo("antecedencia_min_horas", "Antecedência mínima (horas)", "2", "number")}{campo("limite_agendamento_dias", "Agendar até quantos dias", "60", "number")}</div>{config.slug && <div className="mt-6 rounded-xl border border-indigo-200 bg-indigo-50 p-4"><p className="text-xs font-bold uppercase text-indigo-600">Seu link público</p><p className="mt-2 break-all text-sm font-semibold text-indigo-950">{window.location.origin}/agendar/{config.slug}</p><div className="mt-3 flex gap-2"><button type="button" className="btn-secondary py-2" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/agendar/${config.slug}`); toast.success("Link copiado"); }}><Copy className="h-4 w-4"/>Copiar</button><a className="btn-secondary py-2" href={`/agendar/${config.slug}`} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4"/>Visualizar</a></div></div>}</> : <><div className="mb-6"><h2 className="text-xl font-bold">Horários de funcionamento</h2><p className="text-sm text-slate-500">Defina os períodos em que sua empresa normalmente atende.</p></div><div className="divide-y divide-slate-100">{dias.map(([id, nome]) => { const h = config.horarios_funcionamento[id]; return <div key={id} className="grid gap-3 py-4 sm:grid-cols-[1fr_130px_130px] sm:items-center"><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={h.ativo} onChange={(e) => setConfig({ ...config, horarios_funcionamento: { ...config.horarios_funcionamento, [id]: { ...h, ativo: e.target.checked } } })}/>{nome}</label><input type="time" disabled={!h.ativo} className="app-input" value={h.inicio} onChange={(e) => setConfig({ ...config, horarios_funcionamento: { ...config.horarios_funcionamento, [id]: { ...h, inicio: e.target.value } } })}/><input type="time" disabled={!h.ativo} className="app-input" value={h.fim} onChange={(e) => setConfig({ ...config, horarios_funcionamento: { ...config.horarios_funcionamento, [id]: { ...h, fim: e.target.value } } })}/></div>})}</div></>}<div className="mt-8 flex justify-end border-t border-slate-100 pt-6"><button disabled={salvando} className="btn-primary"><Save className="h-4 w-4"/>{salvando ? "Salvando..." : "Salvar configurações"}</button></div></form>}
  </main></AppShell>;
}
