...
export default function Agendamentos() {
  ...
  // ✅ Detalhes controlado por UM id (evita "abrir todos")
  const [detalheAbertoId, setDetalheAbertoId] = useState(null);
  const toggleDetalhes = (id) => setDetalheAbertoId((prev) => (prev === id ? null : id));
          ...
          {agendamentosFiltrados.map((agendamento) => {
            ...
            return (
              <CardAgendamento
                ...
                onMudarStatus={handleMudarStatus}
                onEnviarLembrete={handleEnviarLembrete}
                detalhesAberto={detalheAbertoId === id}
                onToggleDetalhes={() => toggleDetalhes(id)}
              />
            );
          })}
        </AnimatePresence>
          ...
          <div className="w-full max-w-lg bg-white rounded-xl shadow-xl border border-slate-200">
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-slate-600 font-semibold">{agendamentoSelecionado.servico}</div>
              </div>
